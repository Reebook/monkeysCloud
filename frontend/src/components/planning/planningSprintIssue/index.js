...
function PlanningSprintIssue(props) {
  ...
  return (
    <div className='issue-root' onClick={handleClick} data-el_name='btnSelectIssue' data-el_value={JSON.stringify(props.issue)}>
      <div
        className='issue-row'
        style={{
          borderLeft: `3px solid ${props.issue.epic_color}`,
          background: sprintState.selectedIssue.id === props.issue.id ? '#deebff' : null,
        }}
      >
        <div className='monkeys-p-2 d-flex align-items-center'>
          <div className='monkeys-p-2'>
            <input type='checkbox' name='issue-checkbox' id='issue-checkbox-id' />
          </div>
          ...
          <div className='monkeys-p-2'>
            <div
              ...
              style={{
                ...
                color: Color(props.issue.epic_color).luminosity() >= 0.5 ? 'black' : 'white',
              }}
            >
        ...
        <div className='monkeys-p-2 d-flex align-items-center'>
          <div className='monkeys-p-2'>
            <MonkeyAvatar user={props.issue.assignee} key={`issue-assignee-id-${props.issue.assignee.id}`} />
          </div>
          <div
            className='monkeys-p-2'
            style={{
              textDecorationLine: props.issue.status === 'done' ? 'line-through' : 'none',
            }}
          >
            {props.issue.name}
          </div>
          <div className='monkeys-p-2'>
            <FaArrowAltCircleUp
              title={props.issue.priority[0].toUpperCase() + props.issue.priority.slice(1, props.issue.priority.length)}
