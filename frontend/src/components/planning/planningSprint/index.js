  ...
  return (
    <div className='d-flex flex-column monkeys-p-3'>
      <div className='d-flex align-items-center'>
        ...
        <span className='monkeys-text-gray'>{props.sprint.issues.length} issues</span>
      </div>
        ...
        <Fragment>
          ...
          <div className='d-flex align-items-center'>
            {props.sprint.issues.slice(0, 5).map(issue => (
              <MonkeyAvatar user={issue.assignee} key={`assignee-id-${issue.assignee.id}`} />
            ))}
          ...
          <div className='d-flex align-items-center flex-column'>
            {props.sprint.issues.map(issue => (
              <PlanningSprintIssue issue={issue} key={`planning-sprint-issue-id-${issue.id}`} />
            ))}
          </div>
        </Fragment>
      )}
